    ...
    if (items != null && JSON != null) {
        try {
            ...
            for (var i = 0; i < items.length; i++) {
                var item = items[i];
                if (item.itemNo != null && item.name != null && item.price != null && item.quantity != null) {
                  console.log("item.itemNo"+item.itemNo);
                    item = new cartItem(item.itemNo, item.name, item.price, item.quantity);
...
shoppingCart.prototype.addItem = function (itemNo, name, price, quantity) {
    ...
    if (quantity != 0) {
        ...
        // new item, add now
        if (!found) {
            var item = new cartItem(itemNo, name, price, quantity);
            this.items.push(item);
        }
...
shoppingCart.prototype.getTotalPrice = function (itemNo) {
    var total = 0;
    for (var i = 0; i < this.items.length; i++) {
        var item = this.items[i];
        if (itemNo == null || item.itemNo == itemNo) {
...
// get the total price for all items currently in the cart
shoppingCart.prototype.getTotalCount = function (itemNo) {
    var count = 0;
    for (var i = 0; i < this.items.length; i++) {
        var item = this.items[i];
        if (itemNo == null || item.itemNo == itemNo) {
...
shoppingCart.prototype.checkoutPayPal = function (parms, clearCart) {
    ...
    for (var i = 0; i < this.items.length; i++) {
        var item = this.items[i];
        var ctr = i + 1;
        data["item_number_" + ctr] = item.itemNo;
...
shoppingCart.prototype.checkoutGoogle = function (parms, clearCart) {
    ...
    for (var i = 0; i < this.items.length; i++) {
        var item = this.items[i];
        var ctr = i + 1;
        data["item_name_" + ctr] = item.itemNo;
...
// items in the cart
//
function cartItem(itemNo, name, price, quantity) {
    this.itemNo = itemNo;
    this.name = name;
    this.price = price * 1;
